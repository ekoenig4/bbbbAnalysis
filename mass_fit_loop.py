...
directory = 'root://cmseos.fnal.gov//store/user/srosenzw/bbbb_ntuples/fastSim_v5/'
...
# c.Divide(2,1)
filename_save = 'mX_dist_lin_overlay_genmatching.pdf'
...
df = pd.DataFrame(columns=["m_X","m_Y","m_X"])
...
for MX in MX_vals: # Loop through mX values
    ...
    for MY in MY_vals: # Loop through mY values
        ...
        name1 = 'mX distribution, jj-matching but HY-missmatch, mX = {} GeV, mY = {} GeV'.format(MX, MY)
        h1 = TH1F("h1",name1,nbins,0,1400)
        h2 = TH1F("h2",name1,nbins,0,1400)
        h2.SetLineColor(kRed)
        h1.GetXaxis().SetTitle("m_X [GeV]")
        try:
            t.Draw("HH_m >> h1","(gen_H1_b1_matchedflag < 0 || gen_H1_b2_matchedflag < 0 || gen_H2_b1_matchedflag < 0 || gen_H2_b2_matchedflag < 0) && (recoJetMatchedToGenJet1 >= 0 && recoJetMatchedToGenJet2 >=0 && recoJetMatchedToGenJet3 >= 0 && recoJetMatchedToGenJet4 >= 0)") 
            t.Draw("HH_m >> h2","gen_H1_b1_matchedflag >= 0 & gen_H1_b2_matchedflag >= 0 && gen_H2_b1_matchedflag >= 0 && gen_H2_b2_matchedflag >= 0") 
        ...
        # h2 = TH1F("h2",name2,nbins,0,700)
        # try:
        #     t.Draw("H2_m >> h2","(gen_H1_b1_matchedflag < 0 || gen_H1_b2_matchedflag < 0 || gen_H2_b1_matchedflag < 0 || gen_H2_b2_matchedflag < 0) && (recoJetMatchedToGenJet1 >= 0 && recoJetMatchedToGenJet2 >=0 && recoJetMatchedToGenJet3 >= 0 && recoJetMatchedToGenJet1 >= 0)") # Select events with cuts
